...
bool CApp::OnInit()
{
    ...
    strncpy(gameTitle.imagename,"sprites/title.bmp",255);
    gameTitle.load(renderer);
    gameTitle.sfx = Mix_LoadWAV("sounds/level.wav");

    strncpy(gameOver.imagename,"sprites/gameover.bmp",255);
    gameOver.load(renderer);
    gameOver.sfx = Mix_LoadWAV("sounds/gameOver.wav");

    strncpy(win.imagename,"sprites/win.bmp",255);
...
void CApp::initialDraw(){
    ...
    strncpy(backgroundRight.imagename,"sprites/background.bmp",255);
    backgroundRight.load(renderer);
    backgroundRight.setPosition(320,0);

    strncpy(backgroundLeft.imagename,"sprites/background.bmp",255);
    backgroundLeft.load(renderer);
    backgroundLeft.setPosition(0,0);

    strncpy(layerBack.imagename,"sprites/baseLayer.bmp",255);
    layerBack.load(renderer);

    strncpy(layerCosmetic.imagename,"sprites/cosmeticLayer.bmp",255);
    layerCosmetic.load(renderer);

    strncpy(basePlatform.imagename,"sprites/basePlatfomr.bmp",255);
    basePlatform.load(renderer);
    basePlatform.setPosition(0,432);

    strncpy(lowerPlatform.imagename,"sprites/platform.bmp",255);
    lowerPlatform.load(renderer);
    lowerPlatform.setPosition(0,360);

    strncpy(higherPlatform.imagename,"sprites/platform.bmp",255);
    higherPlatform.load(renderer);
    higherPlatform.sfx = Mix_LoadWAV("sounds/hurt.wav");
    higherPlatform.setPosition(0,289);

    strncpy(bridge.imagename,"sprites/bridge.bmp",255);
    ...
    for (int i = 0; i < numPlatform; ++i) {
        //select the sprite
        strncpy(movingPlatforms[i].imagename,"sprites/movingPlatform.bmp",255);
        //load the sprite
    ...
    for (int i = 0; i < numCaves; ++i) {
        //select the sprite
        strncpy(caves[i].imagename,"sprites/cave.bmp",255);
        //load the sprite
        ...
        strncpy(movingPlatforms[pi].imagename,"sprites/movingPlatform.bmp",255);
    ...
    for (int i = 0; i < arrowPerRow; ++i) {
        //select the sprite
        strncpy(arrows[i].imagename,"sprites/arrow.bmp",255);
        //load the sprite
    ...
    strncpy(player.imagename,"sprites/player.bmp",255);
    player.load(renderer);

    strncpy(heart.imagename,"sprites/heart.bmp",255);
    heart.load(renderer);
